...
//    val dataStream: DataStream[(String, Int)] = textDstream.flatMap(_.split("\\s")).filter(_.nonEmpty).map((_, 1)).keyBy(0).sum(1)
    ...
    //对局结算数据流
    val keyed_DStream1: KeyedStream[String, String] = input_Dstream2
      .map(line => {
    ...
    val keyed_DStream2: KeyedStream[String, String] = input_Dstream1.filter(str => str.split(',')(0) != "null")
 ...
 //连接两条流
    keyed_DStream1.connect(keyed_DStream2).process(new CoProcessFunction[String,String,String] {
      ...
      override def processElement2(in2: String, ctx: CoProcessFunction[String, String, String]#Context, out: Collector[String]): Unit = {
        ...
        if(in1==null){
          ...
          //把第二条流的注册到定时器
          ctx.timerService().registerEventTimeTimer(timeValid)
        }else{
          //与第一个流的数据进行关联，然后清空第一个流的数据
          out.collect(in1+in2)
          inputData1.clear()
        }
        //更新第二条流的数据
